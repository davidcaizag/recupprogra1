...
void cargarProductos(Producto productos[], int *numProductos) {
    ...
    *numProductos = 0;
    while (fscanf(file, "%s %d %f %s", productos[*numProductos].nombre, &productos[*numProductos].cantidad, &productos[*numProductos].precio, productos[*numProductos].categoria) == 4) {
        (*numProductos)++;
    }
    fclose(file);
}
